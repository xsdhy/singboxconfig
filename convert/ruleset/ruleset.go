...
// appendGEOIPLines 把 geoip 字段渲染为大写国家/地区代码的 GEOIP 规则行。
func appendGEOIPLines(lines []string, fields map[string]json.RawMessage) []string {
	for _, value := range stringValues(fields, fieldGEOIP) {
		lines = append(lines, string(LineRuleGEOIP)+","+strings.ToUpper(value))
	}
	return lines
}
...
// 输出为远程 URL 引用。tag 与 device 使用 PathEscape，token 使用 query escaping，
// systemHost 先去掉尾部斜杠，确保特殊字符（空格、斜杠、问号等）都能正确还原。
func BuildRuleSetURL(systemHost string, tag string, software entity.Software, device string, token string) string {
	...
	builder.WriteString("/")
	builder.WriteString(string(software))
	builder.WriteString("/")
	builder.WriteString(url.PathEscape(device))
